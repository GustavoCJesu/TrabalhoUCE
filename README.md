...
1. **Entidade** (`domain/entities/`) — o formato do dado retornado.
2. **Interface do repositório** (`domain/repositories/`) — o contrato.
3. **Data source** (`data/datasources/`) — a requisição à API.
4. **Repositório** (`data/repositories/`) — orquestra o datasource (e o token, se autenticado).
5. **Caso de uso** (`application/usecases/`) — a ação.
6. **Registro no container** (`core/config/container.ts`) — monta e expõe a cadeia.
7. **Hook** (`presentation/hooks/`) — estado para a tela.
8. **Tela** (`app/`) — consome o hook e exibe os dados.
